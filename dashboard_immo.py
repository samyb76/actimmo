...
# ── Connexion BDD via Streamlit secrets ───────────────────────────────────────
def get_conn():
...
def load_secteurs():
    with closing(get_conn()) as conn:
        return pd.read_sql("SELECT id, nom FROM secteurs ORDER BY nom", conn)
...
if logo_path:
    ...
    if logo_path.endswith(".png"):
        mime = "image/png"
    elif logo_path.endswith((".jpg", ".jpeg")):
        mime = "image/jpeg"
    else:
        mime = "image/svg+xml"
    ...
    header_content = f"""
        <img src="{logo_src}" alt="Act'Immobilier"
             style="max-height:200px;max-width:100%;object-fit:contain;" />
    """
else:
    header_content = """
        <span style="font-family:'Playfair Display',serif;font-size:1.9rem;color:#8dc63f;">
            🏛 Act'Immobilier
        </span>
    """
...
# ── Filtrage ───────────────────────────────────────────────────────────────────
def chevauche(row):
    ch_min = row["nombre_chambres"] if pd.notna(row["nombre_chambres"]) else None
    ch_max = (
        row["nombre_chambres_max"] if pd.notna(row["nombre_chambres_max"]) else ch_min
    )
...
# ── Tableau ────────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="section-title">📋 Liste des acquéreurs</div>',
    unsafe_allow_html=True,
)
...
def fmt_chambres(row):
    try:
        ch_min = (
            int(row["nombre_chambres"]) if pd.notna(row["nombre_chambres"]) else None
        )
        ch_max = (
            int(row["nombre_chambres_max"])
            if pd.notna(row["nombre_chambres_max"])
            else None
        )
...
# ── CRUD ───────────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="section-title">✏️ Gestion des données</div>',
    unsafe_allow_html=True,
)
            ...
            if not sects_choisis:
                ...
                try:
                    ...
                    for sect_nom in sects_choisis:
                        cur.execute(
                            """
                            INSERT INTO acquereur_secteurs (acquereur_id, secteur_id)
                            VALUES (%s, %s)
                            """,
...
with tab2:
    if len(df) > 0:
        ...
        for _, row in df.iterrows():
            ch_min = (
                int(row["nombre_chambres"]) if pd.notna(row["nombre_chambres"]) else 0
            )
            ch_max = (
                int(row["nombre_chambres_max"])
                if pd.notna(row["nombre_chambres_max"])
            ...
            nom_complet = (
                f"{row['acquereur_prenom'] or ''} {row['acquereur_nom'] or ''}".strip()
            )
            if not nom_complet:
                nom_complet = f"ID {row['id']}"
            ...
            label = f"{nom_complet} — {row['secteurs']} | {int(row['budget']):,} € | {chambres_txt}"
            opts[label] = row["id"]

        sel = st.selectbox(
            "Acquéreur à supprimer",
            ["Choisir un acquéreur"] + list(opts.keys()),
        )
        ...
        if st.form_submit_button("✅ Ajouter", use_container_width=True):
            if not nom_new.strip():
                ...
                try:
                    ...
                    cur = conn.cursor()
                    cur.execute(
                        "INSERT INTO secteurs (nom) VALUES (%s)",
                        (nom_new.strip(),),
                    )
                    conn.commit()
                    st.success(f"✅ Secteur « {nom_new.strip()} » ajouté !")
                    refresh()
